...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            ...
            <a href="/admin/cancelation/create" class="btn btn-success btn-add-new">
               <span>Create Cancelation</span>
            </a>
        @endcan
        </h1>
...
@section('content')
    ...
    <div class = "panel panel-primary">
        <div class = "panel-heading">
            <h3 class = "panel-title">Cancelation Records</h3>
        </div>
        ...
        <div class = "panel-body">
            ...
            <table id="dataTable" class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                ...
                <tr>
                    <td>{{$item->id}}</td>
                    <td>
                        <a href="/admin/customers/{{@$item->booking->customers->sal_customer_id}}">
                        {{@$item->booking->customers->sal_customer_name}}
                        </a>
                    </td>
                    <td>
                        <a href="/admin/plot/show/{{@$item->booking->plots->id}}">
                        {{@$item->booking->plots->name}}
                        </a>
                    </td>
                    <!-- Plot value -->
                    <td>{{number_format($item->booking->amount)}}</td>
                    <!-- Cancelation Fee -->
                    <td>{{number_format($item->fee)}}</td>
                    <!-- Received -->
                    <td>{{number_format($item->booking->payments->PaidAmount)}}</td>
                    <!-- Payable -->
                    <td>{{number_format($item->booking->amount + $item->fee - $item->booking->payments->PaidAmount)}}</td>
                    ...
                    <td>
                        ...
                        <a class="badge badge-success" href="/admin/cancelation/show/{{$item->id}}">View</a>
                        @endif
                        @if(Auth::user()->hasPermission('edit_cancelation'))
                        <a class="badge badge-warning" href="/admin/cancelation/edit/{{$item->id}}">Edit</a>
                        @endif
                        @if(Auth::user()->hasPermission('delete_cancelation'))
                        <a class="badge badge-danger" href="/admin/cancelation/delete/{{$item->id}}">Delete</a>
                        @endif
                        @if(Auth::user()->hasPermission('read_cancelation'))
                        <a class="badge badge-info" href="/admin/cancelation/docs/{{$item->id}}">Docs</a>
                        @endif
