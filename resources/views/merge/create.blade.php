...
@section('content')
    ...
    <div class = "panel panel-primary">
        <div class = "panel-heading">
            <h3 class = "panel-title">Create Merger</h3>
        </div>
        
        <div class = "panel-body">
        <form action = "{{route('merge.store')}}" method ="post" enctype = "multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-md-3">
             ...
             <div class="col-md-3">
                 <div class="form-group" style="margin-top: 30px">
                    ...
                    <input type="text" required  onKeyPress="if(this.value.length==9) return false;" class="form-control" placeholder="ref num" name="ref_num">
                 </div>
             </div>
             <div class="col-md-2">
                 <div class="form-group" style="margin-top: 30px">
                     <label class="control-label">Serial Num <span class="required">*</span> </label>
                    <input type="text" required onKeyPress="if(this.value.length==9) return false;" readonly value="{{$ser_num}}" class="form-control" placeholder="Ser num" name="ser_num">
                 </div>
             </div>
             <div class="col-md-2">
                 <div class="form-group" style="margin-top: 30px">
                     <label class="control-label">Transfer Fee </label>
                     <input required type="text" value="{{$fee}}"  onKeyPress="if(this.value.length==5) return false;"  name="fee" class="form-control">
                 </div>
             </div>
...
<hr>
    <div class="col-md-6">
        <label for="customer">Plot 1</label>
            <select required class="form-control " id="plot_1" name="plot_1"  tabindex="-1" aria-hidden="true">
            ...
            <div class="col-md-6">
                <label for="Plot 2" id="">Plot 2</label>
                    <select required class="form-control select2 plot_2" id="plot_2" name="plot_2"  tabindex="-1" aria-hidden="true">
                        <option value="" >-- Choose</option>
                        @foreach($plots as $item)
                        <option value="{{$item->id}}" >{{$item->name}}</option>
                        @endforeach
            ...
            $(document).ready(function() {
                ...
                $('#customer').change(function(){
                   var customer = $('#customer').val();
                    ...
                    $.ajax({
                        url: "/admin/merge/customer/search",
                        data: {
                            "customerID": customer ,
                            },
                        dataType:"json",
                        ...
                        console.log(data.plots[0].plots.name);
                        ...
                        $('#c_cnic').text(data.cn );
                        $('#plot_1').html();
                        $('#plot_1').html('<option value="" > -- Choose</option>');
                        for( k = 0 ; k < data.plots.length ; k++){
                                $('#plot_1').append('<option value="'+data.plots[k].plots.id+'" >'+data.plots[k].plots.name+'</option>');
                            }
         ...
         $('#plot_1').change(function(){
                ...
                $.ajax({
                    ...
                    type: "get",
                            ...
                            $('.plot_2').html('');
                            $('.plot_2').append('<option value="" >-- Choose</option>');
