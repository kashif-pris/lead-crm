        ...
        <div class="panel panel-defualt">
            ...
            <div class="row">
                ...
                ">
                    ...
                    <table>
                    <h2 style="text-align: center;margin-top: 289px;">({{$bookOrder->plots->block->name}})</h2>
                    ...
                    <tr style="height: 50px !important;">
                        <td>Address: {{$bookOrder->customers->sal_customer_address_1}}</td>
                        <td>Precinct:
        ...
        <div class="card">
            <table class="table">
                ...
                <tbody>
                    ...
                    <form action="{{route('savePAC',['id'=>Request::segment('4')])}}" method="post" enctype="multipart/form-data">
                    @else  
                    <form action="{{route('saveDPAC',['id'=>Request::segment('5')])}}" method="post" enctype="multipart/form-data">

                    @endif
                    @csrf
                        @php 
                        // dd($bookOrder->allotment_Certificate[0]->id);
                            if((count($bookOrder->allotment_Certificate) > 0 ) && empty(Request::segment(5))){
                             
                                $docs = DB::table('form_documents')
                                        ->join('documents','documents.file_id','=','form_documents.id')
                                        ->where('form_documents.form_id',14)
                                        ->where('documents.event_id',14)
                                        ->where('documents.event_value',$bookOrder->allotment_Certificate[0]->id)
                                        ->get();
                            }elseif(!empty(Request::segment(5))){
                                if(!empty(Request::segment(5)) && empty(Request::segment(6))){
                                if(empty(Request::segment(6)) && isset($bookOrder->allotment_Certificate[Request::segment(6)+1]->id)){
                                    $docs = DB::table('form_documents')
                                        ->join('documents','documents.file_id','=','form_documents.id')
                                        ->where('form_documents.form_id',22)
                                        ->where('documents.event_id',14)
                                        ->where('documents.event_value',$bookOrder->allotment_Certificate[Request::segment(6)+1]->id)
                                        ->get();
                        ...
                        <tr>
                            ...
                            <td>
                                @if(!empty($d->file))
                                <a href="/{{$d->path}}/{{$d->file}}" target="_blank"><span class="voyager-eye"> View</span></a>
                                @else 
                                <input type="file" name="file[]" {{$d->is_required}}>
                                @endif
                            </td>
                        </tr>
                       
                        @if((count($bookOrder->allotment_Certificate) > 0 && $i == 0))
                        @if(!empty($bookOrder->allotment_Certificate[0]->file))
                            <tr>
                                <th scope="row"></th>
                                    
                                    <td>
                                        @if(!empty($d->file))
                                        <a href="/{{$bookOrder->allotment_Certificate[0]->file}}" target="_blank">
                                       
                                        <button type="button" class="badge badge-success" >
                                            Generated Allotment File ({{$bookOrder->allotment_Certificate[0]->updated_at}}) 
                                        </button>
                                    </a>
                                    @if(empty($bookOrder->allotment_Certificate[0]->cnic))
                                    <button type="button" class="badge badge-success" data-toggle="modal" data-target="#deliverModal" data-whatever="getbootstrap">Deliver This Certificate</button>
                                    @else  
                                    <button type="button" class="badge badge-success">Delivered at {{date('F j Y g:i a', strtotime($bookOrder->allotment_Certificate[0]->delivered_at))}}</button>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select name="agent" class="form-control select2">
                                    ...
                                    <option value="{{$item->id}}" {{$item->id == $bookOrder->agent_id ? 'selected' : ''}}>{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </td>
                        </tr>
                ...
                @if(Auth::user()->hasPermission('read_allotments') && count($bookOrder->allotment_Certificate) > 0)
                    @if($bookOrder->allotment_Certificate[0]->status != "Approved")
                    
                            
                            <div style="margin-left: 500px">
                                
                                    <a href="javascript:void(0)" onclick="apporveCertificate('{{$bookOrder->allotment_Certificate[0]->id}}','{{Auth::user()->id}}')" class="btn btn-success " class="btn btn-success">Approve</a>
                ...
                @endif
                @if((count($bookOrder->allotment_Certificate) <= 0 || !empty(Request::segment(5))) && empty(Request::segment(6)))
                    <button type="submit"  class="btn btn-success ">Create Request</a>
                @endif
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="post" action="{{route('pac.reject')}}">
            @csrf
            <div class="form-group">
              ...
              @if(count($bookOrder->allotment_Certificate) > 0)

              <input type="hidden" class="form-control" name="id" value="{{$bookOrder->allotment_Certificate[0]->id}}">
              @endif
...
<div class="modal fade" id="deliverModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Delivery Form</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form method="post" action="{{route('pac.deliver')}}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <label for="recipient-name" class="col-form-label right" style="float: left">Subject:</label>
              <input type="text" class="form-control" required id="recipient-name" name="subject">
              <label for="recipient-name"  class="col-form-label right" style="float: left">Attach CNIC Copy:</label>
              @if(count($bookOrder->allotment_Certificate) > 0)

              <input type="hidden" class="form-control" name="id" value="{{$bookOrder->allotment_Certificate[0]->id}}">
              @endif
              <input type="file" class="form-control" required id="path" name="file">
            </div>
            <div class="form-group">
              <label for="message-text" class="col-form-label" style="float: left">Message:</label>
