...
@section('content')
    ...
    <div class = "panel panel-primary">
        <div class = "panel-heading">
            <h3 class = "panel-title">Follow Up List Report</h3>
        </div>
        <div class="row">
        <form action="/admin/followup/date-filter" method="GET" enctype="multipart/data">
        ...
        <table id="example" class="display" style="width:100%">
            ...
            <tbody>
                ...
                @endphp
                 <tr>
                    <td>{{$key+1}}</td>
                    <td>@if(@$item->call_status == 'done')
                        Done <span class="icon voyager-check" style="color:black;"></span>
                        @else
                        <a href="javascript:void(0);" onclick='getMessage({{@$item->id}})' class="btn btn-sm btn-success">Checked</a>
                        @endif
                    </td>
                    <td>{{@$leads->name}}</td>
                    <td>{{@$user->name}}</td>
