...
@section('content')
        ...
        <div class="panel panel-defualt">
            ...
            <div class="row">
                ...
                ">
                    ...
                    <tr style="height: 50px !important;">
                        <td>Contact: {{$bookOrder->customers->phone_1}}</td>
                        <td>Category:</td>
                    </tr>
                    </table>
                ...
                <tbody>
                    ...
                    @csrf
                        ...
                        <tr>
                            ...
                            <td>
                                @if(!empty($d->file))
                                <a href="/{{$d->path}}/{{$d->file}}" target="_blank"><span class="voyager-eye"> </span> View</a>
                                @else 
                        ...
                        @if(!empty($bookOrder->allotment_Certificate[Request::segment(6)+1]->file))
                            <tr>
                                <th scope="row">1</th>
                                <td>
                                    <button type="button" class="badge badge-success">Generated Allotment File  ({{date('F j Y g:i a', strtotime($bookOrder->allotment_Certificate[Request::segment(6)+1]->updated_at))}})</button>

                                     
                                    @if(empty($bookOrder->allotment_Certificate[Request::segment(6)+1]->cnic))
                                    ...
                                    @else  
                                    <button type="button" class="badge badge-success">Delivered at {{date('F j Y g:i a', strtotime($bookOrder->allotment_Certificate[Request::segment(6)+1]->delivered_at))}}</button>
                                ...
                                <td>
                                    @if(!empty($d->file))
                                    <a href="/{{$bookOrder->allotment_Certificate[Request::segment(6)+1]->file}}" target="_blank"><span class="voyager-eye"> View</span></a>
                                    @else 
                                    <input type="file" name="file[]" {{$d->is_required}}>
                                    @endif
                                </td>
  ...
  {{-- @dd($bookOrder->allotment_Certificate[Request::segment(6)+1]->id); --}}
                @if(Auth::user()->hasPermission('read_allotments') && Request::segment(6) != null )
                    @if($bookOrder->allotment_Certificate[Request::segment(6)+1]->status != "Approved" && $bookOrder->allotment_Certificate[Request::segment(6)+1]->status != "Delivered")
                     
                            
                            <div style="margin-left: 500px">
                                   
                                    <a href="javascript:void(0)" onclick="apporveCertificate('{{$bookOrder->allotment_Certificate[Request::segment(6)+1]->id}}','{{Auth::user()->id}}')" class="btn btn-success " class="btn btn-success">Approve</a>
                ...
                @endif
                @if(empty(Request::segment(6)) && Request::segment(6) == null)
            ...
            <div class="modal-body">
              ...
              <form method="post" action="{{route('pac.reject')}}">
                ...
                <div class="form-group">
                  ...
                  <input type="hidden" class="form-control" name="id" value="{{$bookOrder->allotment_Certificate[Request::segment(6)+1]->id}}">
